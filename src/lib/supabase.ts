...
export async function getExistingAttendance(
  ...
  const { data } = await supabase
    .from("attendance")
    .select("student_id,is_present,id")
...
export async function upsertAttendance(
  ...
  const existingMap = new Map(
    (existing ?? []).map((e: any) => [e.student_id, e.id]),
  );
  ...
  for (const u of toUpdate) {
    await supabase
      .from("attendance")
      .update({ is_present: u.is_present, marked_by: userId ?? "admin" })
      .eq("id", u.id);
  }
...
export async function getMonthlyAttendance(
  ...
  return (data ?? []).map((a: any) => ({
    ...a,
    students: stuMap.get(a.student_id) ?? null,
  }));
}
...
export async function getUserByUsername(username: string) {
  const { data, error } = await supabase
    .from("app_users")
    .select(
      "id,username,password_hash,full_name,role,assigned_class,language,is_active",
    )
...
export async function getAllUsers() {
  const { data, error } = await supabase
    .from("app_users")
    .select(
      "id,username,full_name,role,assigned_class,language,is_active,created_at",
    )
    .order("role")
    .order("full_name");
  if (error) throw error;
...
export async function createUser(payload: {
  ...
  const { data, error } = await supabase
    ...
    .select("id,username,full_name,role,assigned_class,language,is_active")
    .single();
  if (error) throw error;
  return data;
}
...
export async function createDonation(
  ...
  await logActivity(
    ...
    "donation",
    data.id,
    { donor: payload.donor_name, amount: payload.amount },
    userId,
  );
  return data;
}
...
const GRADE_WORDS_EN: Record<number, string> = {
  1: "First",
  2: "Second",
  3: "Third",
  4: "Fourth",
...
export function parseClassId(
  raw: string,
): { grade: number; gender: "boys" | "girls" } | null {
  if (!raw) return null;
...
export function classLabel(
  ...
  if (parsed) {
    ...
    if (lang === "ar")
      return `الصف ${GRADE_WORDS_AR[grade]} ${gender === "boys" ? "أولاد" : "بنات"}`;
