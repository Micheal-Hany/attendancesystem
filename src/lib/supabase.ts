...
export async function createStudent(
  payload: { full_name: string; class_id: string; phone?: string; notes?: string; address?: string; birth_date?: string; parent_name?: string; parent_phone?: string },
  userId?: string
) {
  ...
  const insertData: any = {
    ...
    gender: cls?.gender === 'girls' ? 'female' : 'male',
...
// ── VISITATIONS ───────────────────────────────────────────────
export async function getVisitations(classId?: string, limit = 100) {
  let q = supabase.from('visitations').select('*').order('visit_date', { ascending:false }).limit(limit);
  if (classId) {
    const { data: ids } = await supabase.from('students').select('id').eq('class_id', classId);
    if (ids?.length) q = q.in('student_id', ids.map((s:any)=>s.id));
  }
  const { data } = await q;
  return data ?? [];
}

export async function upsertVisitation(payload: {
  student_id: string; visit_date: string; visited_by: string;
  was_home: boolean; notes?: string;
}, userId?: string) {
  const { data, error } = await supabase.from('visitations').upsert(
    payload, { onConflict: 'student_id,visit_date' }
  ).select().single();
  if (error) throw error;
  return data;
}

export async function deleteVisitation(id: string) {
