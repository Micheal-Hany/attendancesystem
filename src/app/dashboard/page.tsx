...
export default function DashboardPage() {
  ...
  async function load() {
    ...
    try {
      const scopeClass = isAdmin
        ? undefined
        : (user?.assigned_class ?? undefined);
      ...
      let attQ = supabase
        .from("attendance")
        .select("student_id,is_present,week_number")
        .eq("month", month)
        .eq("year", year);

      if (scopeClass && studs.length) {
        attQ = attQ.in(
          "student_id",
          studs.map((s: any) => s.id),
        );
      }
      ...
      allAtt
        ...
        .forEach((a: any) => {
          weekMap.set(a.week_number, (weekMap.get(a.week_number) ?? 0) + 1);
        });
      ...
      setLogs(
        (logData ?? []).map((l: any) => ({
          label: `${l.action} ${l.entity_type}${
            l.details?.name ? " — " + l.details.name : ""
          }`,
      ...
      if (isAdmin) {
        ...
        setUpcomingEvents(
          evts.filter((e: any) => e.event_date >= todayStr).slice(0, 3),
        );

        const dons = await getDonations(month, year);

        const total = dons.reduce(
          (s: number, d: any) =>
            s + (d.currency === "EGP" ? Number(d.amount) : 0),
          0,
        );
    ...
    <AppShell>
      <div className="stack" style={{ gap: 14 }}>
        ...
        >
          ...
          />
          <h2 style={{ fontSize: 17, fontWeight: 800, marginBottom: 2 }}>
            {greeting}، {user?.full_name} 👋
          </h2>
          <p style={{ opacity: 0.7, fontSize: 12 }}>
            {monthName(month)} {year} ·{" "}
            {now.toLocaleDateString(isRTL ? "ar-EG" : "en-GB", {
              weekday: "long",
            })}
          </p>
        ...
        <div className="grid-4">
          {statCards.map((c) => (
            <div
              key={c.key}
              className={`card stat-card ${c.color}`}
        ...
        {isAdmin && (
          <div className="grid-2">
            <div
              className="card stat-card stat-amber"
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    {t("total_donations")} — {monthName(month)}
                  </p>
                  <p
                    style={{
                      fontSize: 24,
                  ...
                  >
                    {loading ? "—" : monthDonations.total.toLocaleString()}{" "}
                    <span style={{ fontSize: 13 }}>EGP</span>
                  </p>
                  <p style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>
                    {monthDonations.count}{" "}
                    {lang === "ar" ? "تبرع" : "donations"}
                  </p>
                </div>
                <Heart size={24} color="#16a34a" style={{ opacity: 0.4 }} />
              </div>
            </div>
            <div
              className="card stat-card stat-indigo"
            ...
            >
              ...
              >
                <div>
                  ...
                  <p style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>
                    {lang === "ar" ? "فعالية قادمة" : "events upcoming"}
                  </p>
                </div>
                <Calendar size={24} color="#4338ca" style={{ opacity: 0.4 }} />
              </div>
            </div>
          </div>
        )}
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ fontSize: 9, color: "#94a3b8" }}>
                        {monthName(
                          new Date(ev.event_date + "T12:00:00").getMonth() + 1,
                        ).slice(0, 3)}
                      </span>
                    </div>
